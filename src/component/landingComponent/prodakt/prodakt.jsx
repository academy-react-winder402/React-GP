...
const Prodakt = () => {
  ...
  const getCourseTop = async () => {
    const res = await axios.get("https://classapi.sepehracademy.ir/api/Home/GetCoursesTop?Count=6");
    return res.data;
  };

  const { data } = useQuery("courseTopList", getCourseTop);

  data && console.log(getCourseTop);

  return (
    <div>
      <div className="block sm:hidden">
        <div className="w-[90%] mx-auto mb-36">
          <h2 className="text-2xl font-bold mt-20 text-center"> محصولات </h2>
            ...
            {data?.map((item) => {
              return (
                <div className="shadow-[0px_0px_7px_2px_rgba(0,0,0,0.2)] py-[20px] px-[20px] rounded-3xl w-[315px]">
                  ...
                  <div className="flex justify-between mb-3">
                    <div className="text-white rounded-full text-center bg-blue-700">
                      {" "}
                      {item.title}{" "}
                    </div>
                    {/* <price className="text-red-600"> {item.cost} </price> */}
                  </div>
                  ...
                  <div className="flex justify-between">
                    <Rating name="size-medium" defaultValue={2} />
                    <NavLink to={`/courses/${item.courseId}`} className="w-10 h-10 rounded-full bg-blue-700">
