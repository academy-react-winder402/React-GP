    ...
    <div className="w-full h-full bg-white rounded-xl py-[10px]">
      <div className="flex justify-between w-[95%] mx-auto pb-[20px] border-b-[1px] border-b-black">
        ...
        <div className="flex gap-3">
          <img className="w-8 h-7" src={zangoole} alt="Notification" />
        ...
        <div className="w-[48%] h-full">
          ...
          {slide.map((item) => (
            <div className="relative flex h-[100px] shadow-[0px_0px_10px_2px_rgba(0,0,0,0.2)] rounded-xl mb-[10px] py-[3%] px-[3%]">
              <img className="w-[90px] h-[70px] rounded-xl ml-[15px]" src={item.src} alt={item.name} />
              <div>
                <h3 className="font-bold text-base">{item.name}</h3>
                <h4 className="font-bold text-xs absolute bottom-5">{item.ostadName}</h4>
              </div>
              <span className="absolute left-5 bottom-5">{item.pir} تومان</span>
            </div>
          ))}
        </div>
        <div className="w-[48%]">
