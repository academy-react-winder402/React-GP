...
const Dashboard = () => {
  ...
  const {data} = useQuery('useProfile' , getUserProfile)
  ...
  return (
    <div className="w-[73%] h-full bg-white rounded-xl py-[10px]">
      ...
      <div className="relative flex flex-wrap gap-[10%] border-[1px] border-black rounded-xl w-[95%] h-[150px] mx-auto mt-[10px] pt-[20px] pr-[50px]">
        ...
        <div className="flex gap-3">
          <h2> شماره ملی : </h2>
          <h3> {data?.nationalCode} </h3>
        </div>
        <div className="flex gap-3">
          <h2> ایمیل : </h2>
          <h3> {data?.email} </h3>
        </div>
        <button className="absolute bottom-0 left-0 w-[100px] h-[50px] border-[1px] border-black rounded-tr-xl rounded-bl-xl">
          {" "}
          ویرایش{" "}
        </button>
      </div>
          ...
          {slide.map((item) => {
            return (
              <div className="relative flex h-[100px] shadow-[0px_0px_10px_2px_rgba(0,0,0,0.2)] rounded-xl mb-[10px] py-[3%] px-[3%]">
                <img
                  className="w-[90px] h-[70px] rounded-xl ml-[15px]"
                  src={item.src}
                />
                <div>
                  <h3 className="font-bold text-base"> {item.name} </h3>
                  <h4 className="font-bold text-xs absolute bottom-5">
                    {" "}
                    {item.ostadName}{" "}
                  </h4>
                </div>
                <price className="absolute left-5 bottom-5">
                  {" "}
                  {item.pir} تومان{" "}
                </price>
              </div>
